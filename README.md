I needed a custom buildpack in order to reduce the sludge. Eventually solved it in a different way (changed torch dependency to version which doesn't support GPU and is much smaller).  
However it was a good practice and I'm still running with it. The only effect is echoing the site-packge/* disk usage on coompile time.
